<script lang="ts">
	...
	async function onResize() {
		...
		sketch = await P5Sketch.loadAndRun(data.slug, { darkMode, frameRate: 0, size: dim });
	}
	...
	const onKeyDown = (e: KeyboardEvent) => {
		...
		const recognized = ['Space', 'Slash', 'Enter', 'KeyR', 'KeyM', 'KeyD'];
...
<div class="px-8 md:px-16 xl:px-24 flex flex-col h-screen w-full lg:max-w-6xl xl:min-w-6xl">
	...
	{#if verbose}
		<span>Press Slash to view keyboard shortcuts. Press M to hide this message.</span>
	{/if}
</div>
