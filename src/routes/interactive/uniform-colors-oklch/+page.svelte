<script lang="ts">
	import mappings from '../palette-generator/lib/oklch-color-mapping.json';
	...
	// $: maxChromaForLightnessSlider = map[lightnessSlider].toFixed(4);

	let hueSlider = 27.33;
	let maxChromaForHueSlider = 0.04;
	const maxAvailableChroma = 0.1275; // Math.max(...Object.entries(map).map(([k, v]) => v));
</script>

<h2 class="post-title">Exploring Uniform Colors in OKLCH color space</h2>
...
<p>
	...
	the HSL color space to <a
		href="https://martin.ankerl.com/2009/12/09/how-to-create-random-colors-programmatically/"
		target="_blank">generate uniform color schemes</a
	>. However, despite its straightforwardness, HSL often leads to inconsistent perceptions of color,
	mainly due to its lack of perceptual uniformity. This inconsistency was the catalyst for my
	exploration into an alternative approach, leading me to the OKLCH color space.
</p>
...
<h3>The Quest for Uniformity</h3>
...
<h3>Selecting Lightness and Chroma Values</h3>
<p>
	...
	lightness value and needs to be calculated. I used the following code (<a
		href="https://deno.land/"
		target="_blank">deno</a
	>
	inside a Jupyter notebook) to map lightness to maximum chroma values. There might be an easier way
	to do this, but the following works:
</p>

<CodeDisplay code={data.codeMapper} language="javascript" header="mapper.ts" />

<h3>Creating a Palette with consistent Lightness (and Chroma) while varying Hue</h3>
...
<CodeDisplay code={data.codePaletteHue} header="paletteHue.svelte" />
...
<h3>Creating a Palette with consistent Hue (and Chroma) while varying Lightness</h3>
<p>
...
<CodeDisplay code={data.codePaletteLightness} header="paletteLightness.svelte" />
